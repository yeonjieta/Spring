...
<div class="paging">
	...
	<c:forEach var="i" begin="${paging.begin }" end="${paging.end }">
		<a href="${cpath }/noticeBoard?page=${i }&search=${paging.search}&column=${paging.column}">[${i }]</a>
	</c:forEach>
	<c:if test="${paging.next }">
		<a href="${cpath }/noticeBoard?page=${paging.end + 1}&search=${paging.search}&column=${paging.column}">[다음]</a>
	</c:if>
</div>
